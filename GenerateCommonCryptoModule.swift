...
guard let sdk = SDK(rawValue: CommandLine.arguments[0])?.rawValue else { reportError(message:"SDK must be one of \(SDK.all.map { $0.rawValue })") }
...
if CommandLine.arguments.count > 1 {
    moduleDirectory =  "\(CommandLine.arguments[1])/Frameworks/\(sdk)/CommonCrypto.framework"
